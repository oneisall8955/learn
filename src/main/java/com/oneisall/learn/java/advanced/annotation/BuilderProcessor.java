 ...
 *
 * @author : oneisall
 * @version : v1 2019/5/28 11:42
 */
@SupportedAnnotationTypes({"com.oneisall.learn.java.advanced.annotation.Builder"})
...
public class BuilderProcessor extends AbstractProcessor {
    ...
    public boolean process(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
        ...
        elementsAnnotatedWith.forEach(element -> {
            ...
            element.getEnclosedElements().forEach(field -> {
                if (field.getKind() == ElementKind.FIELD) {
                    ...
                    MethodSpec methodSpec = MethodSpec.methodBuilder("build" + transformedName)
                            .returns(TypeName.get(element.asType()))
                            .returns(ClassName.get("com.oneisall.learn.java.advanced.gen", builderClassName))
                            .addModifiers(Modifier.PUBLIC)
            ...
            try {
                JavaFile.builder("com.oneisall.learn.java.advanced.annotation.gen", typeSpec).build().writeTo(System.out);
                JavaFile.builder("com.oneisall.learn.java.advanced.annotation.gen", typeSpec).build().writeTo(filer);
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
